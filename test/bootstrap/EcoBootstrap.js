...
describe('EcoBootstrap [@group=2]', () => {
  ...
  beforeEach(async () => {
    ...
    bootstrap = await deploy('EcoBootstrap', await owner.getAddress(), numPlaceholders);
  });

  it('allocates 20 placeholder addresses', async () => {
    expect((await bootstrap.NUM_PLACEHOLDERS()).toString()).to.equal(numPlaceholders.toString());
  ...
  it('preserves ownership in the placeholder contracts', async () => {
    ...
    const initializableProxy = await ethers.getContractAt('EcoInitializable', placeholderAddress);
