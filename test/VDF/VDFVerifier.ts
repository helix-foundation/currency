...
describe.only('VDFVerifier [@group=6]', () => {
  const t = 3;
  const xbn = new BN(
  ...
  beforeEach(async () => {
    ...
    isPrimeContract = await deploy(
      'IsPrime'
    ); 
    instanceVDFVerifier = await deploy(
      'VDFVerifier',
      await account.getAddress(),
      isPrimeContract.address
      
    );

    // const VDFVerifier = await ethers.getContractFactory("VDFVerifier")
    // erc20 = await VDFVerifier.deploy(await account.getAddress(), isPrimeContract.address)
    // await erc20.deployed()

   
  });
  describe.only("Primal needs to be set", ()=>{
    it("should reject when primal not set", async () =>{
      await expect(
        instanceVDFVerifier.start(t, '0x0001', {
          gasLimit: 6000000,
        })
      ).to.be.revertedWith(
        'primal must be set'
    ...
    it.only("should reject when primal not set in previous block", async () =>{
      console.log("account : "  + await account.getAddress())
      console.log("contract : "  +  instanceVDFVerifier.address)
      await isPrimeContract.connect(account).setPrimal(bnHex(xbn));
      await expect(
        instanceVDFVerifier.connect(account).start(t, '0x0001', {
          gasLimit: 6000000,
        })
      ).to.be.revertedWith(
        'primal must be set'
    ...
    it("should succeed when primal is set in pervious block", async () =>{
      await isPrimeContract.setPrimal(bnHex(xbn));
  ...
  describe('BigNumbers', () => {
    before(async () => {
      isPrimeContract.setPrimal(bnHex(xbn));
    });
    ...
    it('Rejects malformed bigint words', async () => {
      ...
      await expect(
        instanceVDFVerifier.start(t, bigone, { gasLimit: 6000000 })
