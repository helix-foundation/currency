...
contract MakePoodle is Policy, Proposal {
    ...
    function enacted(address _self) public override {
        ...
        address _switcherCurrencyTimer = MakePoodle(_self).switcherCurrencyTimer();

        Policed(_currencyTimer).policyCommand(_switcherCurrencyTimer, abi.encodeWithSignature("setBordaImpl(address)", _newGovernance));
    }
}
